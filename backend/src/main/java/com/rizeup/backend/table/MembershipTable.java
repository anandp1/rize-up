...
public class MembershipTable {
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if(resultSet.next()){
                    ...
                    while (!resultSet.isAfterLast()) {
                        Membership temp = new Membership(resultSet.getString("M.membership_name"), resultSet.getInt("M.duration"), resultSet.getFloat("M.price"));
                        ArrayList<String> perks = new ArrayList<String>();
                        for(String membership = resultSet.getString("M.membership_name"); membership == resultSet.getString("M.membership_name") && !resultSet.isAfterLast(); resultSet.next()){
    ...
    public Membership getMembership(String name) throws SQLException{
        try (PreparedStatement statement = connection.prepareStatement(
            ...
            try (ResultSet resultSet = statement.executeQuery()) {
                if(resultSet.next()){
                    Membership result = new Membership(resultSet.getString("M.membership_name"), resultSet.getInt("M.duration"), resultSet.getFloat("M.price"));
                    ArrayList<String> perks = new ArrayList<String>();
                    do {
                        perks.add(resultSet.getString("P.perk"));
